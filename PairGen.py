class PairGen:
    def __init__(self, X_train, y_train, X_test, y_test, batch_size=32, step=256, train_val_ratio=0.8, positive_label=1.0, negative_label=0.0):
        import numpy as np
            ...
            while step_cnt < step:
                ...
                while batch_cnt < batch_size:
                    ...
                    if current_label == 1:
                        ...
                        indexArr = np.random.choice(int(len(train_idx[classNum])*train_val_ratio), 2, replace=False)
                        ...
                        indexArr = [np.random.randint(int(len(train_idx[classNum[0]])*train_val_ratio)), 
                                    np.random.randint(int(len(train_idx[classNum[1]])*train_val_ratio))]
        ...
        def generator_valid():
            ...
            while step_cnt < step:
                ...
                while batch_cnt < batch_size:
                    ...
                    if current_label == 1:
                        classNum = np.random.randint(len(train_idx))
                        indexRange = np.arange(int(len(train_idx[classNum])*train_val_ratio)+1, len(train_idx[classNum]))
                        indexArr = np.random.choice(indexRange, 2, replace=False)
                        image_left.append(X_train[train_idx[classNum][indexArr[0]]])
                        image_right.append(X_train[train_idx[classNum][indexArr[1]]])
                        label.append([positive_label])
                    else:
                        classNum = np.random.choice(len(train_idx), 2, replace=False)
                        indexRange = [np.arange(int(len(train_idx[classNum[0]])*train_val_ratio)+1, len(train_idx[classNum[0]])), 
                                      np.arange(int(len(train_idx[classNum[1]])*train_val_ratio)+1, len(train_idx[classNum[1]]))]
                        indexArr = [np.random.choice(indexRange[0]), np.random.choice(indexRange[1])]
                        image_left.append(X_train[train_idx[classNum[0]][indexArr[0]]])
                        image_right.append(X_train[train_idx[classNum[1]][indexArr[1]]])
                        label.append([negative_label])
        ...
        self.ds_train_total = tf.data.Dataset.from_generator(generator_train_total, output_signature=(
            tf.TensorSpec(shape=(2, None, input_shape[1], input_shape[2], input_shape[3]), dtype=tf.float32), 
            tf.TensorSpec(shape=(None, 1), dtype=tf.float32)
        ))
